...
///
/// In each struct variant, if a field has type `Box<enum_name>`, then it is a
/// _child_. Otherwise, it is an _attr_. Recursion into children is
/// automatic, and each child is pushed onto a stack stored on the heap rather
/// than directly calling a method to avoid stack overflow. By default,
/// recursion into attributes (such as `expr: DimExpr` above) is disabled
/// and uses a direct method call, not the aforementioned stack. That is, by
/// default, attributes are simply moved over as-is.
///
