...
pub fn should_skip_attr_ty(ty: &Type) -> bool {
    matches!(ty_as_ident(ty), Some(ty_ident) if matches!(&ty_ident.to_string()[..], "String" | "DebugLoc" | "UBig" | "f64" | "char"))
        || matches!(ty_as_of_ty(ty), Some((ty_ident, arg)) if should_skip_attr_ty(arg) && matches!(&ty_ident.to_string()[..], "Option" | "Box" | "Vec"))
}
